...
> **Current status: internal-alpha code candidate; sanitized-data no-go.** The complete synthetic
> evidence-to-specification path is implemented across three workflow shapes, with structured
> delivery assessments and an objective program scorecard. Live Auth0, AWS, restore,
...
| Internal alpha scorecard                                                                                                                                                         |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![AI-FDE internal alpha scorecard showing three synthetic workflow profiles and the locked conventional baseline](output/playwright/internal-alpha/internal-alpha-scorecard.png) |

The internal-alpha screenshot is generated by `make alpha-rehearsal`. It is browser evidence from a
fresh disposable environment, not a manually composed product mockup.
...
Acme Manufacturing remains the exception-heavy accounts-payable fixture. Northstar Health adds an
employee-onboarding system handoff and Beacon Logistics adds straight-through support triage. The
product supports a named primary workflow per engagement and does not embed accounts-payable
language in generated workflows, economics, or artifacts.

## How the original limitations are addressed

| Original limitation                    | Repository-controlled result                                                                                                                             | Remaining honest boundary                                                                 |
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Only text and Markdown evidence        | PDF, DOCX, CSV, EML, PNG/JPEG, Markdown, and text parsers with bounded, deterministic locators                                                           | No OCR or live source connectors                                                          |
| Deterministic Acme-only extraction     | Transparent fixture patterns now cover ownership, system use, approvals, exceptions, sequence, handoffs, and governance across three synthetic workflows | Production extraction still requires a completed Bedrock evaluation                       |
| One AP workflow pattern                | Exception-heavy AP, multi-system onboarding, and straight-through support triage run through the same product lifecycle                                  | One named primary workflow per engagement remains the deliberate V1 scope                 |
| Base economics only                    | Deterministic low/base/high scenarios with labeled transforms and conservative ordering                                                                  | Realized ROI requires a deployed baseline and observation window                          |
| One Markdown specification             | Atomic seven-artifact packet: PRD, architecture, rules, integrations, controls, evaluation plan, and implementation specification                        | Artifacts remain Markdown and do not execute code                                         |
| Auth0 not live-validated               | PKCE, opaque session, enrollment, expiry, revocation, and contract coverage plus a live-validation runbook                                               | A real tenant record is still external and incomplete                                     |
| Worker, AWS, and Bedrock only proposed | Accepted ADRs, dedicated worker identity, production images, validated Terraform, fail-closed Bedrock adapter, and a release-bound readiness verifier    | The AWS deployment and Bedrock evaluation have not been executed in a live account        |
| Sanitized data disabled                | Retention, export, deletion, RLS, readiness gates, and a bounded pilot runbook are implemented                                                           | Sanitized data correctly remains disabled until the signed external GO                    |
| Coding-agent execution post-V1         | Seven reviewed artifacts create a bounded engineering handoff and preserve the future execution boundary                                                 | Coding-agent dispatch, pull-request generation, and autonomous remediation remain post-V1 |
...
| Extraction      | Provider-neutral contract; transparent deterministic patterns across three synthetic fixtures; Bedrock structured output, exact offsets, fail-closed errors, and run metadata | Live model evaluation and inference-profile pinning not run |
...
| Evaluation      | Durable operator/engineering assessments, objective delivery scorecards, and comparison thresholds across three workflows                                                     | Human alpha cohort and conventional baseline still required |
...
- Three-profile production-mode browser rehearsal with an objective internal-alpha scorecard.
- Readiness unit checks for release digests, ECS rollback/version consistency, and completed
  Bedrock evaluation identity.
...
- three synthetic workflow profiles and reusable workflow primitives;
...
`make seed` is idempotent and loads only synthetic Acme Manufacturing, Northstar Health, and Beacon
Logistics fixtures. `make dev` starts the API, persistent worker, and web application together.
...
## Run the internal alpha rehearsal

Run all three workflow shapes through the real browser and capture the program scorecard:

```bash
make alpha-rehearsal
```

The automated run records three AI-FDE operator assessments and intentionally leaves the
conventional cohort incomplete. Human internal-alpha sessions must supply matched conventional
assessments before the product exposes absolute method differences. Follow the
[internal alpha runbook](docs/runbooks/internal-alpha.md); do not interpret the automated fixture as
a time, ROI, token-cost, or pull-request-cost reduction claim.
...
make alpha-rehearsal  # clean three-workflow internal-alpha browser journey
...
fixtures/{acme,northstar,beacon}/ explicitly synthetic evidence
...
- [Internal alpha runbook](docs/runbooks/internal-alpha.md)
- [Production-equivalent staging runbook](docs/runbooks/production-equivalent-staging.md)
- [Bounded design-partner pilot runbook](docs/runbooks/bounded-design-partner-pilot.md)
- [Internal-alpha implementation plan](docs/plans/2026-08-14-feat-internal-alpha-delivery-system-plan.md)
...
AI-FDE has a credible, locally verified evidence-to-specification foundation across three synthetic
workflow shapes. It is ready for human internal-alpha evaluation, followed by live infrastructure
and provider validation. It is not yet approved for sanitized customer data.

The next correct move is to collect the matched human internal-alpha and conventional assessments,
fix any P0/P1 usability or trust failures, and then complete the external identity, deployment,
recovery, deletion, secret, and extraction evaluations. The next move is not to add coding-agent
autonomy.
